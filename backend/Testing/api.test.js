...
describe('API smoke tests - 10 checks', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      const res = await Promise.race([request(BASE_URL).get('/'), timeoutPromise]);
  ...
  it('9) should respond to forgot password request (will attempt to send OTP)', async () => {
    if (!serverAvailable) { expect(true).toBe(true); return; }
    const res = await request(BASE_URL).post('/api/user/forgotpassword').send({ email: testUser.email });
  ...
  it('10) should allow admin to fetch all bookings (requires admin token)', async () => {
    ...
    if (loginRes.status === 200 && loginRes.body.token) {
      ...
      const res = await request(BASE_URL).get('/api/booking').set('Authorization', `Bearer ${adminToken}`);
